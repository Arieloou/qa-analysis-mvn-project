...
public class BookstoreService {
     ...
     * This dependency enables cascade deletion of books when a bookstore is removed.
     *
     * @param bookService the book service for managing global book references
     */
    public BookstoreService(BookService bookService) {
        if (bookService == null) {
            throw new IllegalArgumentException("BookService must not be null.");
        }
    ...
    public void addBookstore(Bookstore bookstore) {
        if (bookstore == null) {
            throw new IllegalArgumentException("Bookstore must not be null.");
        }
        if (bookstores.containsKey(bookstore.getId())) {
            throw new IllegalArgumentException("A bookstore with this ID already exists.");
        }
    ...
    public void updateBookstore(String id, String name, String address) {
        ...
        if (existing == null) {
            throw new IllegalArgumentException("Bookstore not found with ID: " + id);
        }
     ...
     * @throws IllegalArgumentException if the bookstore is not found or book is null
     */
    public void addBookToBookstore(String bookstoreId, Book book) {
        Bookstore bookstore = bookstores.get(bookstoreId);
        if (bookstore == null) {
            throw new IllegalArgumentException("Bookstore not found with ID: " + bookstoreId);
        }
