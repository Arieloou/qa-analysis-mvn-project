...
class LoanServiceTest {
  ...
  void shouldRetrieveLoanById() {
    ...
    assertTrue(
        loanService.getLoanById(loan.getId()).isPresent());
  }
  ...
  void shouldReturnEmptyForNonExistentLoanId() {
    assertTrue(
        loanService.getLoanById("fake-id").isEmpty());
  }
  ...
  void shouldReturnAllLoans() {
    ...
    loanService.loanBook(book, user,
        LocalDate.now().plusDays(7));
    loanService.loanBook(book2, user,
        LocalDate.now().plusDays(7));
  ...
  void shouldReturnActiveLoansByUser() {
    ...
    Loan loan1 = loanService.loanBook(book, user,
        LocalDate.now().plusDays(14));
    loanService.loanBook(book2, user,
        LocalDate.now().plusDays(14));
    ...
    List<Loan> activeLoans =
        loanService.getActiveLoansByUser(user.getId());
    assertEquals(1, activeLoans.size());
    assertEquals(LoanStatus.ACTIVE,
        activeLoans.get(0).getStatus());
  }
  ...
  void shouldNotReturnLoansFromDifferentUsers() {
    User otherUser = new User("Jane", "jane@test.com",
        UserRole.REQUESTER);

    loanService.loanBook(book, user,
        LocalDate.now().plusDays(14));

    List<Loan> otherLoans =
        loanService.getActiveLoansByUser(otherUser.getId());
