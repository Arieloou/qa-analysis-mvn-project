...
class UserServiceTest {
    ...
    void shouldThrowWhenAddingUserWithDuplicateEmail() {
        userService.addUser(new User("Alice", "same@test.com", UserRole.ADMIN));

        assertThrows(IllegalArgumentException.class,
                () -> userService.addUser(new User("Bob", "same@test.com", UserRole.REQUESTER)));
    }

    @Test
    @DisplayName("Should enforce case-insensitive email uniqueness")
    void shouldEnforceCaseInsensitiveEmailUniqueness() {
        userService.addUser(new User("Alice", "Alice@Test.com", UserRole.ADMIN));

        assertThrows(IllegalArgumentException.class,
                () -> userService.addUser(new User("Bob", "alice@test.com", UserRole.REQUESTER)));
    }
    ...
    void shouldReturnEmptyWhenUserNotFound() {
        assertTrue(userService.getUserById("fake-id").isEmpty());
    }

    @Test
    @DisplayName("Should return all users")
    void shouldReturnAllUsers() {
        userService.addUser(new User("A", "a@test.com", UserRole.ADMIN));
        userService.addUser(new User("B", "b@test.com", UserRole.REQUESTER));
    ...
    void shouldUpdateUserFieldsSuccessfully() {
        User user = new User("Old", "old@test.com", UserRole.ADMIN);
        userService.addUser(user);

        userService.updateUser(user.getId(), "New", "new@test.com");

        User updated = userService.getUserById(user.getId()).orElseThrow();
    ...
    void shouldThrowWhenUpdatingNonExistentUser() {
        assertThrows(IllegalArgumentException.class,
                () -> userService.updateUser("fake-id", "N", "e@test.com"));
    }

    @Test
    @DisplayName("Should throw exception when updating to a taken email")
    void shouldThrowWhenUpdatingToTakenEmail() {
        User user1 = new User("Alice", "alice@test.com", UserRole.ADMIN);
        User user2 = new User("Bob", "bob@test.com", UserRole.REQUESTER);
        userService.addUser(user1);
        userService.addUser(user2);

        assertThrows(IllegalArgumentException.class,
                () -> userService.updateUser(user2.getId(), "Bob", "alice@test.com"));
    }
    ...
    void shouldAllowUpdatingUserWithTheirOwnEmail() {
        ...
        assertDoesNotThrow(
                () -> userService.updateUser(user.getId(), "Alice Updated", "alice@test.com"));
    }
