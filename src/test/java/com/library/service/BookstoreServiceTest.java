...
class BookstoreServiceTest {
  ...
  void shouldAddBookstoreSuccessfully() {
    Bookstore store = new Bookstore("Main Library",
        "100 Center St");
    bookstoreService.addBookstore(store);

    Optional<Bookstore> found =
        bookstoreService.getBookstoreById(store.getId());
  ...
  void shouldReturnEmptyWhenBookstoreNotFound() {
    assertTrue(bookstoreService
        .getBookstoreById("fake-id").isEmpty());
  }

  @Test
  @DisplayName("Should return all bookstores")
  void shouldReturnAllBookstores() {
    bookstoreService.addBookstore(
        new Bookstore("Store A", "Addr A"));
    bookstoreService.addBookstore(
        new Bookstore("Store B", "Addr B"));
  ...
  void shouldUpdateBookstoreFieldsSuccessfully() {
    Bookstore store = new Bookstore("Old Name",
        "Old Address");
    bookstoreService.addBookstore(store);

    bookstoreService.updateBookstore(store.getId(),
        "New Name", "New Address");

    Bookstore updated = bookstoreService
        .getBookstoreById(store.getId()).orElseThrow();
  ...
  void shouldThrowWhenUpdatingNonExistentBookstore() {
    assertThrows(IllegalArgumentException.class,
        () -> bookstoreService.updateBookstore(
            "fake-id", "N", "A"));
  }
  ...
  void shouldDeleteBookstoreSuccessfully() {
    ...
    assertTrue(deleted);
    assertTrue(bookstoreService
        .getBookstoreById(store.getId()).isEmpty());
  }
  ...
  void shouldReturnFalseWhenDeletingNonExistentBookstore() {
    assertFalse(
        bookstoreService.deleteBookstore("fake-id"));
  }
  ...
  void shouldCascadeDeleteBooks() {
    ...
    bookstoreService.addBookToBookstore(
        store.getId(), book1);
    bookstoreService.addBookToBookstore(
        store.getId(), book2);
    ...
    assertTrue(
        bookService.getBookById(book1.getId()).isEmpty());
    assertTrue(
        bookService.getBookById(book2.getId()).isEmpty());
  }
  ...
  void shouldNotAffectOtherBookstoresBooksOnCascadeDelete() {
    ...
    assertTrue(
        bookService.getBookById(bookA.getId()).isEmpty());
    assertTrue(
        bookService.getBookById(bookB.getId()).isPresent());
  }
  ...
  void shouldAddBookToBookstoreAndGlobalService() {
    ...
    assertTrue(
        bookService.getBookById(book.getId()).isPresent());
  }
